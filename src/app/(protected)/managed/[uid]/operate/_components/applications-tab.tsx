...
// Human labels for the 1–5 lifestyle sliders are now provided via t() in BehaviourPanel.
...
function BehaviourPanel({ profile }: { profile: OperateTenantProfile }) {
  ...
  function scaleLabel(v: number | null): string {
    if (v == null) return t('operate.scaleNotSet');
    const keys = [
      null,
      'operate.scaleVeryLow',
      'operate.scaleLow',
      'operate.scaleMedium',
      'operate.scaleHigh',
      'operate.scaleVeryHigh',
    ] as const;
    if (v < 1 || v > 5) return `${v}`;
    return `${t(keys[v] as string)} (${v}/5)`;
  }

  function boolLabel(v: boolean | null): string {
    if (v == null) return t('operate.boolNotSet');
  ...
  return (
    <div className="mt-3 rounded-md border border-border bg-background p-3">
      <p className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
        {t('operate.behaviourSectionTitle')}
      </p>
      <div className="grid grid-cols-2 gap-x-4 gap-y-2 sm:grid-cols-4">
        <InfoPair label={t('operate.behaviourCleanliness')} value={scaleLabel(profile.cleanlinessLevel)} />
        <InfoPair label={t('operate.behaviourNoise')} value={scaleLabel(profile.noiseLevel)} />
        <InfoPair label={t('operate.behaviourSleepSchedule')} value={scaleLabel(profile.sleepSchedule)} />
        <InfoPair label={t('operate.behaviourSociability')} value={scaleLabel(profile.sociability)} />
        <InfoPair label={t('operate.behaviourGuests')} value={scaleLabel(profile.guests)} />
        <InfoPair label={t('operate.behaviourSmoker')} value={boolLabel(profile.isSmoker)} />
        <InfoPair label={t('operate.behaviourPets')} value={boolLabel(profile.hasPets)} />
        <InfoPair label={t('operate.behaviourCooks')} value={boolLabel(profile.cooksOften)} />
      </div>
      {profile.description && (
    ...
    <article className="rounded-lg border border-border bg-surface p-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            {occupationBits.length > 0
              ? occupationBits.join(' · ')
              : t('operate.appNoProfileDetails')}
          </p>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {application.appliedAt
              ? t('operate.appApplied', { date: dateFormatter.format(application.appliedAt) })
              : t('operate.appAppliedUnknown')}{' '}
            · {t('operate.appRoom')} <span className="font-mono">{application.roomId || '—'}</span>
          </p>
      ...
      {profile ? (
        <>
          ...
          >
            {showBehaviour ? t('operate.appHideBehaviour') : t('operate.appShowBehaviour')}
          </button>
          {showBehaviour && <BehaviourPanel profile={profile} />}
        </>
      ) : (
        <p className="mt-2 text-xs italic text-muted-foreground">
          {t('operate.appNoProfile')}
        </p>
      )}
    ...
    <Overlay onClose={pending ? () => {} : onClose}>
      <div className="w-full max-w-md rounded-xl border border-border bg-surface p-6 shadow-xl">
        <h2 className="text-lg font-semibold text-foreground">
          {decision === 'accept' ? t('operate.responseAcceptTitle') : t('operate.responseDeclineTitle')}
        </h2>
        <p className="mt-2 text-sm text-muted-foreground">
          {decision === 'accept' ? (
            <>
              {t('operate.responseAcceptDesc', { statusField: "«status='active'»" })}
            </>
          ) : (
            <>
              {t('operate.responseDeclineDesc', { statusField: "«status='cancelled'»" })}
            </>
          )}{' '}
          {t('operate.responseAdminStamp', { field: '_impersonatedByAdminUid' })}
        </p>

        {decision === 'decline' && (
          <div className="mt-5">
            <Field label={t('operate.responseReasonLabel')} required hint={`${reason.length} / 1000`}>
              <textarea
                rows={4}
                maxLength={1000}
                value={reason}
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          >
            {pending
              ? decision === 'accept'
                ? t('operate.responseAcceptingButton')
                : t('operate.responseDecliningButton')
              : decision === 'accept'
                ? t('operate.appAcceptButton')
                : t('operate.appDeclineButton')}
          </button>
